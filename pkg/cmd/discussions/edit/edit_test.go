...
func TestNewCmdEdit(t *testing.T) {
	tests := []struct {
		...
		// errors (missing args, missing required --org, bad number) surface.
		{name: "no args", args: []string{}, wantErr: true},
		{name: "invalid number", args: []string{"abc", "--org", "my-org", "--title", "T"}, wantErr: true},
		{name: "valid with title", args: []string{"42", "--org", "my-org", "--title", "T"}, wantErr: false},
		{name: "valid with body", args: []string{"42", "--org", "my-org", "--body", "B"}, wantErr: false},
		{name: "valid with category", args: []string{"42", "--org", "my-org", "--category", "C"}, wantErr: false},
		{name: "no changes", args: []string{"42", "--org", "my-org"}, wantErr: false},
		{name: "missing org", args: []string{"42", "--title", "T"}, wantErr: true},
	}
...
func TestEditRunUpdatesOrgDiscussion(t *testing.T) {
	...
	if gotPath != "/api/v5/orgs/my-org/discuss/42" {
		t.Fatalf("path = %s, want /api/v5/orgs/my-org/discuss/42", gotPath)
	}
...
func TestEditRunUpdatesOrgDiscussionBodyAndCategory(t *testing.T) {
	t.Setenv("GC_TOKEN", "test-token")
