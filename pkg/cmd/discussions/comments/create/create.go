// Package create implements the discussions comments create command (org scope).
package create
...
// CreateOptions configures the discussions comments create command (org scope).
...
// NewCmdCreate creates the discussions comments create command (org scope).
func NewCmdCreate(f *cmdutil.Factory, runF func(*CreateOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create <number>",
		Short: "Create a comment on an organization discussion",
		Long: heredoc.Doc(`
			Create a comment on a GitCode organization discussion via the v5 API
		...
		Example: heredoc.Doc(`
			...
			$ echo "Comment text" | gc discussions comments create 42 --org my-org --body-file -

			# Output as JSON
			$ gc discussions comments create 42 --org my-org --body "Comment" --json
		`),
...
func createRun(opts *CreateOptions) error {
	...
	c, err := api.CreateOrgDiscussionComment(client, opts.Org, opts.Number, &api.CreateOrgDiscussionCommentOptions{
		MdContent: body,
	})
	if err != nil {
