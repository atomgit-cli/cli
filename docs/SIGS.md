...
- 通过 **SIG 讨论通道**承载异步讨论与决策记录（Phase 1 为带 `sig/<name>` label 的 issue；discussion 分类开通后迁移，见 sig-governance.md §6.2）

SIG **不**替代 `spec/` 中的任何规则，也不替代现有的 issue / PR workflow。

## 当前 SIG 列表

| SIG | 范围 | 状态 | 注册的 Agent | 讨论通道（Phase 1） |
|-----|------|------|--------------|-----------|
| **sig/api** ⚠️ | `api/`、`api-doc/` — GitCode API 客户端、endpoint 对接、api-doc 同步 | incubating | claude-code, trae-ai | 带 `sig/api` label 的 issue |
| **sig/cli-ux** ⚠️ | `pkg/cmd/`、`pkg/cmdutil/`、`pkg/output/`、`pkg/iostreams/` — 命令交互、`--json` 输出、错误提示、退出码 | incubating | claude-code, trae-ai | 带 `sig/cli-ux` label 的 issue |
| **sig/governance** ⚠️ | `spec/`、`docs/`、`AGENTS.md`、`CLAUDE.md`、`CONTRIBUTING.md` — 规范演进、文档治理、AI 协作边界 | incubating | claude-code, trae-ai | 带 `sig/governance` label 的 issue |
...
>
> 三个试点 SIG 当前均为 `incubating`（试运行）状态，按 sig-governance.md §3 的退出条件达标后转 `active`（试点节奏见 §9）。
...
3. **参与讨论**：到对应 SIG 的讨论通道发帖（Phase 1 为带 `sig/<name>` label 的 issue）

### 作为 reviewer / maintainer

1. triage 新 issue 时，**默认只打 `scope/*` label**；各 SIG 活跃度统计映射的 `scope/*` 集合以 `spec/governance/sigs/*.yaml` 的 `scope_labels` 字段为准（未被映射的 `scope/*` 不计入任何 SIG 活跃度）
2. 仅当 issue 需要 SIG 治理介入时（跨 SIG 协调、SIG 治理变更、活跃度统计、SIG 路线图讨论），才**额外**打 `sig/*` label；拿不准主 SIG 时先打一个，并在 comment 中提及其他相关 SIG
3. 跨 SIG 议题使用带 `sig/governance` label 的 issue，由 `sig/governance` 协调
...
1. 先在该 SIG 范围内提交 ≥ 2 个被合并的 PR，或持续参与该 SIG 讨论通道
2. 在对应 SIG 的讨论通道下自荐（Phase 1 为带 `sig/<name>` label 的 issue）
...
**Q：SIG lead 不响应怎么办？**
A：按 sig-governance.md §4.4"SIG lead 缺位处理"执行：先在该 SIG 讨论通道（带 `sig/<name>` label 的 issue）下提醒；连续 ≥ 2 周无响应时，用带 `sig/governance` label 的 issue 提出，由仓库 maintainer 介入。
...
A：先看 `docs/rfcs/README.md` 的对照表——"大到一个 issue / 讨论通道装不下"的设计走 RFC 流程；拿不准时先开 issue，由 maintainer 判断是否升级为 RFC。
